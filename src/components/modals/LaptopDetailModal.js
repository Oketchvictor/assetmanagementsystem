...
import { FiMonitor, FiMapPin, FiMove, FiUserPlus, FiTool, FiX } from 'react-icons/fi';

const LaptopDetailModal = ({ show, onHide, asset, onViewMap, onAssign, onTransfer, onReport }) => {
  ...
  const getLocationBadge = (location, type) => {
    ...
    if (location.includes('Stores')) cls = 'stores';
             ...
             location.includes('Workshop')) cls = 'offsite';
    ...
    >
      <Modal.Header style={styles.modalHead}>
        <Modal.Title style={styles.modalTitle}>
          <FiMonitor style={{ color: '#4F9EF8' }} /> {asset.model} — <span style={styles.mono}>{asset.tag}</span>
        </Modal.Title>
