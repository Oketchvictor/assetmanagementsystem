...
// PDF Report Generation - Simplified Working Version
const generatePDFReport = (reportType, data, filters) => {
  ...
  yPosition += 5;
  doc.text(`Generated by: ${userName}`, 14, yPosition);
  yPosition += 5;
  doc.text(`Seovo Solutions Asset Management System`, 14, yPosition);
  yPosition += 5;
  ...
  switch (reportType) {
    case 'asset-inventory':
      headers = ['Tag', 'Name', 'Category', 'Serial', 'Location', 'Status', 'Value'];
    ...
    case 'asset-assignment':
      ...
      break;
    case 'maintenance':
      headers = ['Asset Tag', 'Asset Name', 'Issue Type', 'Severity', 'Status', 'Reported Date'];
      tableData = data.map(item => [
        item.asset_tag,
        item.asset_name,
        item.issue_type,
      ...
      break;
    case 'transfer-history':
      headers = ['Asset Tag', 'Asset Name', 'From', 'To', 'Transferred By', 'Date'];
      tableData = data.map(item => [
        item.asset_tag,
        item.asset_name,
        item.from_location,
  ...
  if (tableData.length > 0 && headers.length > 0) {
    doc.autoTable({
      ...
      margin: { top: 60, bottom: 30, left: 14, right: 14 }
    });
  }
      ...
      tableHtml = `
        <table class="report-table">
          ...
          <tbody>
            ${data.map(item => `
              <tr>
                <td style="font-family: monospace;">${item.tag}</td>
                <td>${item.name}</td>
      ...
      tableHtml = `
        <table class="report-table">
          ...
          <tbody>
            ${data.map(item => `
              <tr>
                <td style="font-family: monospace;">${item.asset_tag}</td>
    ...
    case 'maintenance':
      tableHtml = `
        <table class="report-table">
          <thead>
            <tr>
              <th>Asset Tag</th><th>Asset Name</th><th>Issue Type</th><th>Severity</th><th>Status</th><th>Reported Date</th>
            </thead>
          <tbody>
            ${data.map(item => `
              <tr>
    ...
    case 'transfer-history':
      tableHtml = `
        <table class="report-table">
          <thead>
            <tr>
              <th>Asset Tag</th><th>Asset Name</th><th>From</th><th>To</th><th>Transferred By</th><th>Date</th>
            </thead>
          <tbody>
            ${data.map(item => `
              <tr>
    ...
    case 'category-summary':
      tableHtml = `
        <table class="report-table">
          <thead>
            <tr>
              <th>Category</th><th>Total</th><th>Active</th><th>In Stock</th><th>Repair</th><th>Missing</th>
            </thead>
          <tbody>
            ${data.map(item => `
              <tr>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .status-repair { background: #fff3e0; color: #ed6c02; }
    .status-missing { background: #ffebee; color: #d32f2f; }
    .severity-low { background: #e8f5e9; color: #2e7d32; }
    .severity-medium { background: #e3f2fd; color: #1976d2; }
    .severity-high { background: #fff3e0; color: #ed6c02; }
...
<head>
  ...
  <style>
    ...
    th { background: #00E5A8; color: #07101F; padding: 10px; border: 1px solid #ccc; }
